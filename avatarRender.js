...
(function () {
  ...
  async function load(db, uid, opts) {
    var results = await Promise.all([
      ...
      // Columns enumerated deliberately: anon's SELECT grant on cosmetic_items is
      // column-scoped and EXCLUDES admin_notes, rejection_reason,
      // contract_artist_id, staged_asset_paths and contract_status. A select('*')
      // here would ask for those and fail outright for logged-out visitors.
      db.from('cosmetic_items').select('id,name,description,image_url,slot,artist_id,is_marketplace_item,created_at,lines_url,shadow_url,highlight_url,fill_regions,z_index,is_base_item,is_randomizer_item,rarity_weight,updated_at,quill_price,total_sales,is_secret_unlock,tag_slug,character_id,status,submitted_at,asset_paths,ember_unlock_price,work_id'),
